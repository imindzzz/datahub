...
export default function FieldBusinessRelation({ expandedField, editableFieldInfo }: Props) {
    const isSchemaEditable = React.useContext(SchemaEditableContext);
    const urn = useMutationUrn();
    const refetch = useRefetch();
    const schemaRefetch = useSchemaRefetch();
    const [updateBusinessRelationMutation] = useUpdateBusinessRelationMutation();
    ...
    const sendAnalytics = () => {
        analytics.event({
            ...
            actionType: EntityActionType.UpdateSchemaBusinessRelation,
            entityType,
            entityUrn: urn,
        });
    };
    ...
    const generateMutationVariables = (updatedBusinessRelation: string) => ({
        variables: {
            input: {
    ...
    const displayedDescription = editableFieldInfo?.businessRelation || expandedField.businessRelation;

    return (
        <>
            <DescriptionWrapper>
                <div>
                    <SectionHeader>Business Relation</SectionHeader>
                ...
                {isModalVisible && (
                    <UpdateDescriptionModal
                        title={displayedDescription ? 'Update business relation' : 'Add business relation'}
                        description={displayedDescription || ''}
                        original={expandedField.businessDescription || ''}
                        onClose={() => setIsModalVisible(false)}
                        onSubmit={(updatedBusinessRelation: string) => {
                            message.loading({ content: 'Updating...' });
                            updateBusinessRelationMutation(generateMutationVariables(updatedBusinessRelation))
